...
module.exports.likesList = (req, res, next) => {
  Like.find({ user: req.currentUser })
    ...
    .then((likes) => res.json(likes.map((like) => like.adoption)))
    .catch(next);
};
...
module.exports.likes = (req, res, next) => {
  ...
  Like.findOneAndDelete({ adoption: petId, user: req.currentUser })
    .then((like) => {
      ...
      return Like.create({ adoption: petId, user: req.currentUser }).then((newLike) =>
        Pet.findById(newLike.adoption).then((pet) => {
          ...
          return Dislike.findOneAndDelete({ adoption: petId, user: req.currentUser }).then(() =>
